...
Native hard lock remains available to combat rules, but its camera-attachment side effect is corrected using the game's detach transition after lock/target events and before/after the player tick. Already detached cameras perform no repeated detach call. The selected enemy remains intact. Camera evaluation consumes an atomic opaque owner identity validated on the game thread; it performs no UObject reads, registry lookups or Lua calls and does not require the gameplay thread. Owner deletion, menu/cinematic rejection, master disable and shutdown revoke that identity. Camera diagnostics use atomic counters and are included in the existing aggregate log when Logging is enabled.
