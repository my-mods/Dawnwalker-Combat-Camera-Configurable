...
## Last-attacker targeting

The optional `lockLastAttacker` toggle appends one boolean to the settings ABI and defaults to false. The additional hook uses `PlayerCombatComponent::ReactToHit` at RVA `0x5e1c800`, with ABI `void(combat, animationData, attackData, response)`. The player combat vtable slot `+0x8e8` at `0x7d64960` and the complete 1,070-byte function are validated before installation. The native function already belongs to the scoped code contract. Its `InAttackData +0x28` is the attacker's combat component; component `+0xa8` gives its actor. Native block, parry and omniblock reactions use this same incoming-hit route. Original reaction parameters and output pass through unchanged.

The game-thread hook copies only validated indexed identities while the player's lock is already active. It never retains animation/attack/response structs, allocates a weak serial or calls the target picker within a hit reaction. Hits coalesce into one pending request, serviced after the existing native player tick. Pending state expires after one second of gameplay time if targeting remains ineligible. The request consumes the shared 50 ms selection budget and uses the native switch's in-range list with at most two single-candidate primary/fallback checks. The camera-angle rejection is disabled only for this attacker, while native predicates and occlusion still apply. Failure preserves the current selection and does not retry. A hit from the current target establishes priority without any selection pass.

A successful switch holds that enemy against camera-directed requests. Controller slowdown retains its existing cadence and camera modes retain their own policies. Pending, held and in-progress selection identities use six additional slots in the existing deletion listener. Native assignment of an initially zero serial is accepted, with deletion watches armed throughout selection to reject address/index reuse; the current serial is captured when priority is established. Unlock, disable, option changes, owner replacement and invalid contexts clear the feature state; ordinary target loss releases priority. Smooth temporary-loss recovery retains priority, while a new valid attacker can supersede it. Expired or cancelled recovery releases it. No new timer, global discovery or configuration polling is added. Logging aggregates received hits, switches, rejections and selection time; its disabled path does not collect those counters or timestamps.
